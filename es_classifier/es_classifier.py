...
def eval_model(es, config, count, out_result=True):
    logger.info(config)
    ...
    for i, (agent, path) in enumerate(config["agents"].items()):
        ...
        for query in progressbar(data):
            res = inference(es, query, count, config)
            rank = 0
            precisions = 0
...
def inference(es, query, count, config):
...
def parameter_grid_search(es, config, count):
    ks = config["k"]
    ...
    for k in ks:
        for weighting in weightings:
            ...
            results.append({"k": k, "weighting": weighting, "result": eval_model(es, eval_config, count, out_result=False)})
    sorted_res = sorted(results, reverse=True, key=lambda res: res["result"]["accuracy"])
    for res in sorted_res:
        logger.info("k: {}\tweighting: {}\taccuracy: {}".format(res["k"], res["weighting"], res["result"]["accuracy"]))
    if config["out_dir"]:
        folder_name = config.get("folder_prefix", "")+"_"
        path = os.path.join(config["out_dir"], folder_name+datetime.datetime.now().strftime("%b-%d-%Y_%H-%M"))
        os.makedirs(path, exist_ok=True)
...
if __name__ == "__main__":
    ...
    if args.inference:
        inference(es, args.query, count, config)
    elif args.eval:
        eval_model(es, config, count)
    elif args.grid_search:
        parameter_grid_search(es, config, count)