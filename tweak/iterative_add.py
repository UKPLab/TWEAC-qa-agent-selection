...
def extend():
    ...
    for i, agent in enumerate(extend_agents):
        ...
        if "test" in source_config:
            source_config["test"]["agents"][agent] = data_config["test"]["agents"][agent]
        ...
        if "sampling" in config:
            if config["sampling"] == "simple":
                config["train"]["truncate"] = max(1, config["train"]["extend_truncate"]//(len(config["all_agents"])-1))
